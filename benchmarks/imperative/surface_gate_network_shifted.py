...
def gate_bounds(name: str, current_time: float):
    low, high = INITIAL_DOORS[name]
    shift_time, dy = SHIFTS[name]
    if current_time >= shift_time:
        return low + dy, high + dy
    return low, high


def gate_open(name: str, current_time: float) -> bool:
    shift_time, _ = SHIFTS[name]
    return current_time >= shift_time
...
def enforce_gate(sphere: Sphere, previous_x: float, current_time: float, left_gate: str, right_gate: str):
    ...
    if crossed_left:
        if not gate_open(left_gate, current_time) or not point_in_gate(sphere.y, gate_bounds(left_gate, current_time)):
            sphere.x = LEFT_WALL_X + sphere.radius
            if sphere.vx < 0.0:
                sphere.vx = 0.0

    if crossed_right:
        if not gate_open(right_gate, current_time) or not point_in_gate(sphere.y, gate_bounds(right_gate, current_time)):
