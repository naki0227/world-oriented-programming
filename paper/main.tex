...
It now also includes a first shifted-gate network slice, where several entities can resolve through different translated apertures at different frontiers in the same connected-space family.
It now also includes a first path-tube slice, where one declared path segment becomes an admissible motion corridor through an explicit geometry law rather than through procedural closest-point code.
...
\begin{table}[t]
  ...
  \begin{tabular}{lrrrr}
    ...
    surface gate shifted network & 51 & 88 & 0.765 & 17 \\
    path tube & 18 & 54 & 0.944 & 5 \\
    \bottomrule
  \end{tabular}
\end{table}
...
Alongside those two geometry lines, a third family is now beginning to appear: explicit path and trajectory primitives. The current prototype can already declare a named path segment with one width parameter and then keep a sphere inside that admissible motion tube through an explicit \texttt{inside\_tube(...)} law. That moves the project beyond regions and surfaces alone, because one world can now state a trajectory corridor directly instead of encoding closest-point projection and tangent-velocity repair as procedural update logic.
...
It is now also beginning to connect to path-like geometry, since one small trajectory corridor can already be declared as a named segment plus an explicit \texttt{inside\_tube(...)} law.
