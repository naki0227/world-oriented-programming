...
The first geometry slice now underway is a visibility law, where line-of-sight between entities is treated as a world-level condition rather than as ad hoc update logic. The current prototype now also supports multi-occluder visibility, so line of sight can be invalidated by a small declared set of blocking regions rather than by a single hard-coded obstacle.
