...
\begin{table}[t]
  ...
  \begin{tabular}{lrrrr}
    ...
    visibility corridor world & 26 & 64 & 0.885 & 8 \\
    \bottomrule
  \end{tabular}
\end{table}
...
The first geometry slice now underway is a visibility law, where line-of-sight between entities is treated as a world-level condition rather than as ad hoc update logic. The current prototype now also supports multi-occluder visibility and a first corridor-style visibility world, so line of sight can be shaped by a small declared set of blocking regions rather than by a single hard-coded obstacle.
