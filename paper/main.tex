...
In a small Phase I extension, a scene can declare multiple candidate velocities for one entity together with soft scores, while the existing law layer still acts as a hard admissibility filter.
...
The structured report now exposes both the candidate inventory seen before simulation and the candidate resolution performed at the initial frontier.
...
The new underdetermined-world slice is likewise intentionally narrow: it currently supports only single-entity candidate resolution with one simple scoring model.
