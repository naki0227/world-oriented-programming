...
\subsection{Toward an Operational Semantics}

The semantics work now underway sharpens this prototype intuition into an explicit staged model.
The current formulation distinguishes five semantic layers:
...
Candidate events are selected from that frontier by an ordering rule that combines earliest-time selection with a semantic priority and a deterministic tie-breaker.
...
The next transition is then factored into event firing and admissibility enforcement.
In the admissible case, the world continues after any required repair.
In the inadmissible case, execution reaches contradiction at the same semantic frontier instead of falling back to an out-of-band exception model.
This framing also gives a clean reading to the runtime's current activity labels: fired, repaired, and contradicted.

Most importantly, it lets the paper state a stronger semantic claim than before:
...
The present prototype does not yet provide a theorem-level development, but it now has the structure needed for a real semantics section rather than only an execution metaphor.
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{../figures/viewer-04-contradiction-report-paper.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{../figures/two_body_collision-xy.png}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{../figures/viewer-03-roundtrip-result-paper.png}
