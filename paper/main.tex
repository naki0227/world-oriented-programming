...
\begin{figure*}[t]
  ...
  \begin{minipage}[t]{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../figures/viewer-visibility-01-clear-run.png}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../figures/viewer-visibility-02-occluded-preset.png}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../figures/viewer-visibility-03-occluded-run.png}
  \end{minipage}
...
The prototype can also now keep a deferred entity unresolved across more than one observed frontier while other entities continue to evolve.
It can also resolve such a deferred tie at a later frontier when the scene explicitly requests a later convergence point.
It can also let a newly active preference guide that later convergence, so a deferred tie may resolve for a semantic reason stronger than the prototype's default lexical tie-break.
It can now also let later score updates reshape that convergence, so underdetermined worlds need not be committed forever to the initial soft ranking declared at time zero.
...
It can now also let later law updates reshape admissibility itself, so a continuation that was previously blocked by a hard law may later become selectable without abandoning the world-oriented formulation.
It can now also stagger such re-convergence across multiple entities, so the world-level observation status changes across frontiers instead of collapsing in a single step.
