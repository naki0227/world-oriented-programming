...
The current evaluation scaffold now also includes a first visibility pair, where \texttt{visible(A, B)} is contrasted with imperative line-of-sight checking against an occluding region.
...
They are included to show that the evaluation is already reproducible and that the current corpus consistently shifts specification effort away from loops, branches, and explicit update bookkeeping.
...
It can also participate in action preference, so the same world can prefer a pursuit-like continuation when line of sight is clear and a search-like continuation when an occluding region blocks visibility.
...
Figure~\ref{fig:flagship-convergence} shows the current flagship case.
Two entities begin with ambiguous candidate actions while two moving targets pass through a corridor-like visibility structure.
At the first frontier both candidate-bearing entities remain unresolved; at the second, one entity has converged; at the third, the world-level observation is determinate.
The important point is not the animation itself, but the reportable sequence of semantic frontiers: unresolved with two ambiguous entities, unresolved with one ambiguous entity, and then determinate.

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.9\textwidth]{../figures/visibility_coordination_flagship-xy.png}
  \caption{Flagship visibility-coordination scenario. The panels show semantic observation frontiers at $t=0$, $t=1$, and $t=2$; the timeline records staggered convergence from two ambiguous candidate-bearing entities, to one, to a determinate world report.}
  \label{fig:flagship-convergence}
\end{figure*}

\begin{table*}[t]
  \centering
  \caption{Manual reading of the flagship scenario against a favorable library-style Python baseline. The baseline is allowed compact world-building calls; the remaining distinction concerns semantic identity and report continuity.}
  \label{tab:flagship-manual-comparison}
  \begin{tabular}{@{}L{0.17\textwidth}L{0.25\textwidth}L{0.25\textwidth}L{0.23\textwidth}@{}}
    \toprule
    Axis & \texttt{sekai} flagship & Library-style Python baseline & Reading \\
    \midrule
    Law identity &
    Speed laws live in a \texttt{constraint:} block. &
    Laws are calls such as \texttt{world.law(...)}. &
    Python can name laws, but lawhood is an API convention. \\
    Candidate structure &
    The \texttt{action:} block groups candidates, deferral, resolution, and preferences. &
    Candidate and preference calls are listed separately. &
    \texttt{sekai} separates action-world structure from geometry and constraints. \\
    Observation frontiers &
    The \texttt{observe:} block lists semantic frontiers. &
    Observation uses \texttt{world.observe(...)} calls. &
    Both request snapshots; \texttt{sekai} makes observation a source section. \\
    Report continuity &
    One source produces constraints, activities, resolutions, timeline, snapshots, viewer data, and the figure. &
    The library must reproduce the report contract. &
    The stronger claim is source--runtime--report--viewer continuity. \\
    \bottomrule
  \end{tabular}
\end{table*}
...
The first geometry slice now underway is a visibility law, where line-of-sight between entities is treated as a world-level condition rather than as ad hoc update logic.
