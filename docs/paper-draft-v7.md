...
- Human-centered computing -> Visual languages
- Computing methodologies -> Simulation environments

## Keywords

World-Oriented Programming, declarative world description, spatial-temporal programming, constraint-first execution, visual-logical interface, local synchronization
...
1. move the manuscript into the ACM SIGPLAN conference template
2. replace figure-insertion notes with actual figures and captions in the typeset layout
3. convert the reference list into the exact venue bibliography style
4. strengthen the related-work section with a few more archival sources, especially around visual mathematics and language design
5. polish prose for concision and submission length
