...
"#;
    ...
    #[test]
    fn shifted_gate_network_can_resolve_entities_at_staggered_frontiers() {
        let source = r#"
sphere A
sphere B
plane left_wall
...
normal(right_wall) = (-1, 0, 0)
offset(right_wall) = -4
min(left_door_a) = (-4.5, 5, -1)
max(left_door_a) = (-3.5, 7, 1)
min(right_door_a) = (3.5, 5, -1)
max(right_door_a) = (4.5, 7, 1)
min(left_door_b) = (-4.5, -6, -1)
max(left_door_b) = (-3.5, -4, 1)
min(right_door_b) = (3.5, -6, -1)
max(right_door_b) = (4.5, -4, 1)
action:
    candidate_velocity(A, wait_a) = (0, 0, 0) score 5
    candidate_velocity(A, enter_left_a) = (-3, 0, 0) score 5
    candidate_velocity(A, enter_right_a) = (3, 0, 0) score 5
    ...
    prefer_candidate_if_gate_open(B, enter_right_b, right_door_b)
constraint:
    through_gate_shift_after(A, left_wall, left_door_a, 1, 0, -4, 0)
    through_gate_shift_after(A, right_wall, right_door_a, 4, 0, -4, 0, clamp)
    through_gate_shift_after(B, left_wall, left_door_b, 4, 0, 3, 0, clamp)
    through_gate_shift_after(B, right_wall, right_door_b, 2, 0, 3, 0)
observe:
    snapshot at 0
    snapshot at 1
    snapshot at 2
